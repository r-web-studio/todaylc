...
bot.catch((err) => {
  ...
  const ctx = err.ctx;
  ctx.reply(ctx.t?.('errors.unknown') || 'An unexpected error occurred. Please try again.')
    .catch((e) => console.error('Failed to reply in error handler:', e));
});
...
if (config.env === 'production' && config.bot.webhookUrl) {
  ...
  const baseUrl = config.bot.webhookUrl.replace(/\/+$/, '').replace(/\/webhook$/, '');
